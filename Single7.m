...
for k=1:cs
    ...
    for rseed=1:n
        rng(rseed);      % 设置随机种子
        d(rseed)= lognrnd(m,v);      %生成当天的货物总重量
        if (d(rseed)+B0)<c
...
save d d
save JL_Single7_n1000    revenue  Lf   revenue_m c d  tran_T n  fare


%% 收益随仓容变化曲线
load JL_Single7_n100
figure
hold on
h1=plot(c,revenue_m,'k-','LineWidth',1);
load JL_Single7_n1000
h2=plot(c,revenue_m,'r-','LineWidth',1);
grid on
legend([h1 h2],'100天','1000天','Location','best')
xlabel('仓容');
ylabel('平均收益');
title('平均收益随仓容变化曲线')
ylim([-1E6 2E5])
